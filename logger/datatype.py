...
class OrderedValue(DataValue):
    def __cmp__(self, cmpvalue):
        if not isinstance(cmpvalue, OrderedValue):
            raise TypeError(
                "no ordering relation is defined between types %s and %s" % (self.type, type(cmpvalue).__name__))
...
class CountableValue(OrderedValue):
    def __sub__(self, cmpvalue):
        if not isinstance(cmpvalue, CountableValue):
            raise TypeError(
                "no countable relation is defined between types %s and %s" % (self.type, type(cmpvalue).__name__))
        if self.type != cmpvalue.type:
...
class ContinuousValue(OrderedValue):
    ...
    def __sub__(self, cmpvalue):
        if isinstance(cmpvalue, DataValue):
            raise TypeError(
                "no countable relation is defined between types %s and %s" % (self.type, type(cmpvalue.type).__name__))
...
class BinaryType(ContinuousType):
    ...
    def isvalidprimitivevalue(self, value):
        return isinstance(value, (str, int))  # str or byte. NOT UNICODE!
...
class FixedLengthString(CountableType):
    ...
    def toPrimitive(self, value, rounding=0):
        ...
        if len(value) > self.length:
            ...
            value = value + (self.length - len(value)) * ' '
...
class HexType(CountableType):
    ...
    def toPrimitive(self, value, rounding=0):
        ...
        if isinstance(value, int):
            ...
            assert isinstance(value, str)  # str or byte. NOT UNICODE!
        if len(value) > self.length:
            raise ValueError("Can not pack 0x%s of length %d in HexData of length %d" % (
                self.toPrintable(value), len(value), self.length))
        elif len(value) < self.length:
...
class RangeType(OrderedType):
    ...
    def __cmp__(self, cmpvalue):
        ...
        return self.type.compare(self.min.value, cmpvalue.min.value) or self.type.compare(self.max.value,
                                                                                          cmpvalue.max.value)
    ...
    def difference(self, value):  # NEED REVIEW
        ...
        if self._cmpminmin(value) < 0:
            ranges.append(type(self)(self.min, value.min, itemtype=self.itemtype, interval=self.interval,
                                     mininclusive=self.mininclusive, maxinclusive=not value.mininclusive))
        # Remaining piece of self, higher than value.max
        if self._cmpmaxmax(value) > 0:
            ranges.append(type(self)(value.max, self.max, itemtype=self.itemtype, interval=self.interval,
                                     mininclusive=not value.maxinclusive, maxinclusive=self.maxinclusive))
    ...
    def __str__(self):
        return "%s(%s,%s)" % (
            type(self).__name__, self.type.toPrintable(self.min.value), self.type.toPrintable(self.max.value))
...
class CountableRange(RangeType, CountableValue):
    ...
    def __sub__(self, cmpvalue):
        ...
        if self.type != cmpvalue.type:
            raise TypeError("no countable relation is defined between types %s and %s" % (self.type, cmpvalue.type))
        return self.type.diff(self.min.value,
                              cmpvalue.min.value)  # TODO: pass (self.min, cmpvalue.min) (DataValue instead of value)
    # def diff(self, value1, value2):
    #    """value1 - value2."""
    #    # assert self.countable
