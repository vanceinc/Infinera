...
class DebugOutput(modules.output.BaseOutput):
    ...
    def printSubject(self, subject):
        ...
        for var in variables:
            ...
            if isinstance(value, types.MethodType):
            ...
            elif isinstance(value, modules.xmlns.RDFObject):
                vartype = "rdfobj"
            elif isinstance(value, modules.xmlns.XMLNamespace):
                vartype = "xmlns"
                ...
                vartype = "file"
            elif isinstance(value, modules.rangeset.RangeSet):
                vartype = "range"
    ...
    def printNamespaceOverview(self):
        ...
        for namespace in modules.xmlns.GetNamespaces():
            self.write(40 * "-")
            self.printSubject(namespace)
            objectcount += len(namespace.elements)
        self.write(40 * "-")
        self.write("%d objects in %d namespaces" % (objectcount, len(modules.xmlns.GetNamespaces())))

    def printObjectOverview(self):
        objectcount = 0
        classlist = modules.xmlns.GetRDFClasses(sortkey=classSortKey)
        for klass in classlist:
            self.write (40 * "=")
            self.write("%ss:" % klass.__name__)
            objectlist = modules.xmlns.GetAllRDFObjects(klass, exactclass=True)
    ...
    def output(self, subject=None):
        logger = logging.getLogger("modules.output")
        if subject != None:
...
def classSortKey(subject):
    ...
    if issubclass(subject, modules.layers.Layer):
        return "1"+subject.__name__
    # elif issubclass(subject, InterfaceLayer):
    #     return "2"+subject.__name__
    elif issubclass(subject, modules.layers.AdaptationFunction):
        return "3"+subject.__name__
    elif subject == modules.elements.Device:
        return "4"+subject.__name__
    elif issubclass(subject, modules.elements.Device):
        return "5"+subject.__name__
    elif issubclass(subject, modules.elements.SwitchMatrix):
        return "6"+subject.__name__
    elif subject == modules.elements.Interface:
        return "7"+subject.__name__
    elif issubclass(subject, modules.elements.ConnectionPoint):
