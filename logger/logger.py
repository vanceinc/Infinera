#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""logger module: adds a handler to the logger starting with 'network'. It will write this to file.
...
class Logger(object):
    ...
    def readPrevLineCount(self, outfile):
        if not isinstance(outfile, (str,)):
            return
        try:
            infile = open(outfile, 'r+b')
            contents = infile.readlines()
            infile.close()
            self.prevcount = 0
            for logline in contents:
                logline = logline.split('\t', 4)
                if (len(logline) > 3) and logline[2].isdigit() and (int(logline[2]) >= self.loglevel) and logline[
