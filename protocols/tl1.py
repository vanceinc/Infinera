...
# local modules
from protocols.base import BaseLangInput, BaseIOInput, BaseAsyncInput, BaseSyncInput
from protocols.emulate import *
...
def ParseSectionBlock(block):
    ...
    """
    regex = {'name': '[a-zA-Z0-9]+', 'safe_char': '[^";:,]', 'qsafe_char': '[^"]'}
    ...
    # double quotes (") or escape double quotes (\")
...
class TL1IOInput(BaseIOInput):
    ...
    port = 23  # TL1-RAW port
    hasecho = False  # device returns an echo?

    def __init__(self, hostname, port=None, timeout=None):
    ...
    def connect(self):
        try:
            ...
            raise NetworkException(
                "Problem connecting to host ('telnet %s %d')" % (self.hostname, self.port))

        time.sleep(0.01)
        self.writetolog(self.terminal.read_very_eager(), input=True)
        # self.terminal.read_very_eager()
    ...
    def sendcommand(self, string):
        ...
        stale results."""
        self.writetolog(string.encode('utf8'), input=True)
        logger = logging.getLogger("protocols")
        logger.debug("Sending command %s" % (repr(string)))
        # self.acquireIOlock()
        self.terminal.write(string.encode('utf8'))
        # self.releaseIOlock()
        if self.hasecho:
            ...
            if expectstring not in echostring:
                ...
                raise TimeOut(
                    "Did not receive echo of command %s, but got %s." % (repr(string), repr(echostring)))
    ...
    def readmessage(self, timeout):
        ...
        if self.terminator not in resultString:
            logger.error(
                "Did not receive termination string %s in TL1 result %s." % (repr(self.terminator), repr(resultString)))
            raise TimeOut("Did not receive termination string %s in %d seconds in TL1 result %s." % (
        ...
        if not resultString.endswith(self.terminator):
            ...
            raise TimeOut("no response %s in %s from %s in %d seconds (timeout=%d sec)." % (
                repr(self.terminator), repr(resultString), self.hostname, time.time() - endtime + timeout, timeout))
        logger.debug("Received %d bytes of data" % len(resultString))
        return str(resultString, 'utf-8')


class TL1LanguageInput(BaseLangInput):
    """LanguageInput class part, which is knowledge about the format of TL1 input and
    ...
    def authorize(self):
        ...
        try:
            ...
            # self.disconnect()
            raise NetworkException(
                "Password failed when connecting to %s@%s" % (self.username, self.hostname))
    ...
    def statusOK(self, status, command=""):
        """Checks the status. returns True is the status is a successful,
        or raises a CommandFailed, possible with additional information if not.
        """
        # status = [responsetype, status, comments]
        if status[1] != "COMPLD":
            raise CommandFailed(
                "Commmand %s failed: status=%s, reason: %s" % (command, status[1], status[2]))
    ...
    def makeCommand(self, command):
        ...
        Returns a tuple (identifier, command string). The identifier is the ctag."""
        self.acquireMemLock()
        ctag = self.ctag
        self.ctag += 1
        self.releaseMemLock()
        ...
        for line in resultLines:
            ...
            if statusmatch:
                if ctag == None:
                    ...
                    status = statusmatch.group(3)  # warning: may be more then one word! (e.g. "COMPLD" or "DENY" or
                    # "REPT ALM CRS")
                    skiplines = False
                    ...
                    logger.warning(
                        "Ignoring TL1 output with ctag %s, since the output of ctag %s is not finished (we can only "
                        "handle output one-by-one)." % (
                ...
                if skiplines:
                    if ctag is None:
                        logger.error(
                            "Haven't receive a valid status line yet. Thus skip TL1 result line %s" % repr(match))
                    else:
                ...
                logger.debug("Skip uknown TL1 line %s" % repr(line))
        if ctag is None:
            raise MalformedIO(
                "Could not find valid response header (e.g. 'M  123 COMPLD') in response %s" % repr(resultString))
            # NOTE: we actually like to include the associated command that was send out, but we don't have that
            # information. However, experience showed that command is often unrelated to the alarm. So we leave it as
            # it is.
    ...
    def isAutonomousType(self, identifier, status):
        ...
        if responsetype == 'M':
            ...
            raise MalformedIO(
                "Received an unknown message type '%s' (only understand M, A and *) with an identifier %s from %s" % (
                    responsetype, identifier, self.getTarget()))


# Note: first parent takes precedence over other parent classes.
class SyncTL1Input(TL1IOInput, TL1LanguageInput, BaseSyncInput):
    pass


# Note: first parent takes precedence over other parent classes.
class AsyncTL1Input(TL1IOInput, TL1LanguageInput, BaseAsyncInput):
    pass


class TL1EmulatorInput(FileIOInput, TL1LanguageInput, BaseSyncInput):
