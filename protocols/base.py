# -*- coding: utf-8 -*-
"""An internal module of protocols with abstract base classes.
...
represented in UTF-8. Use pynt.xmlns.UTF8 if you need help convert."""
...
from protocols.exceptions import CommandFailed, NetworkException, TimeOut


class BaseIOInput(object):
    """Base I/O input. Abstract class, forming a third part of the BaseInput class, along with BaseLangInput and
    BaseCommandInput """
    timeout = 180  # default time-out in seconds
    ...
    def disconnect(self):
        """closes the I/O connection. You shouldn't call it more than once. Sets the actually I/O object to None,
        if any """
        pass
    ...
    def sendcommand(self, string):
        ...
        raise NotImplementedError(
            "BaseIOInput.sendcommand() is an abstract method. please override in %s" % type(self).__name__)
    ...
    def readmessage(self, timeout):
        ...
        without checking validity. The result MUST be a UTF-8 encoded string.
        Should raise an TimeOut in case more than timeout seconds have been passed."""
        raise NotImplementedError(
            "BaseIOInput.readmessage() is an abstract method. please override in %s" % type(self).__name__)
        resultString = ""
        self.writetolog(resultString, output=True)
        logger = logging.getLogger("protocols")
        logger.debug("Received %d bytes of data" % len(resultString))
        return str(resultString, 'utf-8')


class BaseLangInput(object):
    """Base Language input. Abstract method, forming a third part of the BaseInput class, along with BaseIOInput and
    BaseCommandInput """
    username = ""
    password = ""
    terminator = b";"  # string that signifies the end of a response message
    prompt = b">"  # string that signifies the start of an input message
...
class BaseSyncInput(object):
    ...
    def getmessage(self, identifier, timeout):
        ...
        This function is blocking, although it may give a timeout, if nothing was returned in time.
        ...
        while True:
            ...
            if autotype:
                ...
                if autotype in self.autocallbacks:
                    ...
                    logger.info("Sending autonomous message (type %s, identifier %s) to %s" % (
                    ...
                    logger.warning(
                        "Skipping unhandled autonomous message (type %s, identifier %s)" % (autotype, residentifier))
    ...
    def send_and_receive(self, command, timeout):
        ...
        # Check if RTRV is in the status, if not pass to statusOK
        if status[1] != "RTRV":
    ...
    def isCorrectCallback(self, callback):
        ...
        if isinstance(callback, types.FunctionType):
            argnames = callback.__code__.co_varnames
    ...
    def callbackCommand(self, command, callback, timeout=None):
        ...
        # ("Callback function %s has not the proper argument list: %s(resultlines) or %s(resultline, status=None)",
        # (callback.func_name,callback.func_name,callback.func_name))
        if timeout is None:
            timeout = self.timeout
        (resultlines, status) = self.send_and_receive(command, timeout)
...
class BaseAsyncInput(BaseSyncInput):
    ...
    dorun = False  # thread signal to keep running, or to stop.
    ...
    def send_and_receive(self, command, timeout):
        ...
        if cmdidentifier != residentifier:
            raise CommandFailed("Result identifier %s does not match command identifier %s for command %s." % (
                residentifier, cmdidentifier, command))
        return resultlines, status
    ...
    def addIdentifierCallback(self, identifier, callback, timeout=None):
        ...
        if identifier in self.callbacks:
            raise NetworkException(
                "A command with identifier %s was already sent. Can't use the same identifier more than once in asynchronous mode." % identifier)
        logger = logging.getLogger("protocols")
    ...
    def getmessage(self, identifier, timeout):
        ...
        if identifier in self.callbacks:
            raise AssertionError(
                "getmessages() should not be called with an identifier (%s) present in self.callbacks" % identifier)
        endtime = time.time() + timeout
        ...
        if identifier not in self.messages:
            raise TimeOut(
                "identifier %s not found in messages within %d seconds. Available identifiers: %s" % (
                    identifier, timeout, str(list(self.messages.keys()))))
        self.acquireMemLock()
    ...
    def processMessage(self, message):
        ...
        if (autotype != False):
            ...
            if autotype in self.autocallbacks:  # specific callback function
                callback = self.autocallbacks[autotype]
                logger.info("Sending autonomous message (type %s, identifier %s) to %s()" % (
                    autotype, identifier, callback.__name__))
                self.callback(callback, resultlines, status=status)
            elif True in self.autocallbacks:  # catch-all callback function
                callback = self.autocallbacks[True]
                logger.info("Sending autonomous message (type %s, identifier %s) to %s()" % (
        ...
        if callback:
            ...
            if identifier in self.messages:
                raise CommandFailed(
                    "Can't append result with identifier %s: a result with the same identifer already exists." % identifier)
    ...
    def stopThreads(self, timeout=None):
        ...
        if len(self.messages) > 0:
            logger.error("Unprocessed messages left in queue with id %s, after stopping listener thread" % str(
    ...
    def acquireLoglock(self):
        ...
        while True:
            ...
            if time.time() > endtime:
                raise TimeOut(
                    "Thread %s is unable to get a log lock in 10 seconds." % (threading.currentThread().getName()))
            time.sleep(0.05)
        return gotlock
...
class BaseInput(BaseIOInput, BaseLangInput, BaseAsyncInput):
    """A base input class, consisting of three parts working together:
